...
static void acpirsdptr(void)
{
	...
	printk("FUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUCK\n");
	printk("/* RSDP */ struct Rsdp Rsdp = {%08c, %x, %06c, %x, %p, %d, %p, %x}\n", 
...
printk("fadt.smicmd is %d\n", fadt.smicmd);
	if (fadt.smicmd == 0) {
		//fmtinstall('G', Gfmt);
printk("PRTINT THW  FUCKJA\n");
		acpirsdptr();
		if (fadt.smicmd == 0) {
			return -1;
		}
