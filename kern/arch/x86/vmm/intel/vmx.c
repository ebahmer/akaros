...
static const struct vmxec cb2ec = {
	...
	.set_to_1 = (SECONDARY_EXEC_VIRTUALIZE_APIC_ACCESSES |
...
static void setup_msr(struct vmx_vcpu *vcpu) {
	...
	vmcs_write64(IO_BITMAP_B, PADDR(&io_bitmap[ARRAY_SIZE(io_bitmap) / 2]));
...
int intel_vmm_init(void) {
	...
	io_bitmap = (unsigned long *)get_cont_pages(13, KMALLOC_WAIT);
	if (!io_bitmap) {
		printk("Could not allocate msr_bitmap\n");
		kfree(msr_bitmap);
		return -ENOMEM;
	...
	memset(&io_bitmap[0], 0xff, 2 * PAGE_SIZE);
	...
	__vmx_disable_intercept_for_io(io_bitmap, 0x70);
	__vmx_disable_intercept_for_io(io_bitmap, 0x71);
	if ((ret = ept_init())) {
